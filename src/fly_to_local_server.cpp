...
namespace fly_to_local {
	...
	as_(NULL) {
		...
		ros::NodeHandle action_nh("fly_to_local");
...
geometry_msgs::PoseStamped FlyToLocal::goalToFCUFrame(const geometry_msgs::PoseStamped& goal_pose_msg) {
	...
	tf::Stamped<tf::Pose> goal_pose, fcu_pose;
	...
	try{
		tf_.transformPose(fcu_frame, goal_pose, fcu_pose);
	}
	catch(tf::TransformException& ex) {
		ROS_WARN("Failed to transform the goal pose from %s into the %s frame: %s",
			goal_pose.frame_id_.c_str(), fcu_frame.c_str(), ex.what());
		return goal_pose_msg;
	}

	geometry_msgs::PoseStamped fcu_pose_msg;
	tf::poseStampedTFToMsg(fcu_pose, fcu_pose_msg);
	return fcu_pose_msg;
}
	...
	while(n.ok())
	{
		if(as_->isPreemptRequested()) {
			if(as_->isNewGoalAvailable()) {
				...
				ROS_DEBUG_NAMED("fly_to_local","fly_to_local has received a goal of x: %.2f, y: %.2f", goal.pose.position.x, goal.pose.position.y);
				current_goal_pub_.publish(goal);

			}
			else { // if we've been preempted explicitly, shut things down
				resetState();

				// notify the ActionServer that we've successfully preempted
				ROS_DEBUG_NAMED("fly_to_local","FlyToLocal preempting the current goal");
				as_->setPreempted();
...
bool FlyToLocal::executeCycle(geometry_msgs::PoseStamped& goal) {
	...
	//$ update feedback with curent position
